...
const StreamingMessage = ({ 
  ...
  useEffect(() => {
    if (!isStreaming && isComplete === false) {
      setIsComplete(true);
      setShowCursor(false);
      if (onStreamingComplete) {
        onStreamingComplete(displayedContent);
      }
    }
  }, [isStreaming, isComplete, displayedContent, onStreamingComplete]);
  ...
  const formatMessageContent = (content) => {
    ...
    let formattedContent = content
      ...
      .replace(
        /⚠️\s*(.*?)(?=\n|$)/g,
        '<div class="bg-orange-50 border-l-4 border-orange-400 p-3 my-3 rounded-r-lg"><div class="flex items-center"><span class="orange-600 mr-2">⚠️</span><span class="font-medium text-orange-800">Warning:</span></div><p class="text-orange-700 mt-1">$1</p></div>'
      )
      .replace(
        /ℹ️\s*(.*?)(?=\n|$)/g,
          ...
          >
            ...
            >
              ...
              >
                ...
                {/* Streaming cursor */}
                {showCursor && message.role === "ai" && (
                  <span 
                    ref={cursorRef}
                    className="inline-block w-0.5 h-5 bg-blue-500 ml-1 blink"
                  />
                )}
              </div>
            ...
            >
              ...
              {message.role === "ai" && !message.isError && isComplete && (
