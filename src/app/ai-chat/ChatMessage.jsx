...
const ChatMessage = ({ message, handleSuggestionClick, userProfile }) => {
  const formatMessageContent = (content) => {
    ...
    let formattedContent = content
      ...
      .replace(/`(.*?)`/g, '<code class="bg-gray-100 text-gray-800 px-2 py-1 rounded-md text-sm font-mono border border-gray-200">$1</code>')
      // Enhanced headers with icons and styling
      .replace(/^#{3}\s+(.*)$/gm, '<h3 class="text-lg font-semibold mt-6 mb-3 text-gray-800 flex items-center"><span class="w-2 h-2 bg-blue-500 rounded-full mr-2"></span>$1</h3>')
      .replace(/^#{2}\s+(.*)$/gm, '<h2 class="text-xl font-semibold mt-6 mb-3 text-gray-800 flex items-center"><span class="w-3 h-3 bg-blue-600 rounded-full mr-2"></span>$1</h2>')
      .replace(/^#{1}\s+(.*)$/gm, '<h1 class="text-2xl font-bold mt-6 mb-4 text-gray-900 flex items-center"><span class="w-4 h-4 bg-blue-700 rounded-full mr-2"></span>$1</h1>')
      // Enhanced bullet points with custom icons
      .replace(/^[-•]\s+(.*)$/gm, '<li class="flex items-start mb-2 ml-2"><span class="flex-shrink-0 w-2 h-2 bg-blue-500 rounded-full mt-2 mr-3"></span><span class="text-gray-700">$1</span></li>')
      // Enhanced numbered lists
      .replace(/^(\d+)\.\s+(.*)$/gm, '<li class="flex items-start mb-2 ml-2"><span class="flex-shrink-0 w-6 h-6 bg-blue-100 text-blue-700 rounded-full text-xs font-semibold flex items-center justify-center mt-0.5 mr-3">$1</span><span class="text-gray-700">$2</span></li>')
      ...
      // Quote blocks
      .replace(/^>\s+(.*)$/gm, '<blockquote class="border-l-4 border-gray-300 pl-4 py-2 my-3 bg-gray-50 italic text-gray-600 rounded-r-lg">$1</blockquote>')
    ...
    formattedContent = formattedContent.replace(/(<li class="flex items-start mb-2 ml-2">.*?<\/li>)/g, (match) => {
  ...
  const getPersonalizedAvatar = () => {
    ...
    return (
      <div className={`flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center shadow-lg ring-2 ring-white ${
        message.isError
  ...
  const getPersonalizedSuggestions = () => {
    ...
    } else if (userProfile?.fitnessGoal === 'muscle-gain' && message.type === 'ai') {
      suggestions = suggestions.map(suggestion => {
        if (suggestion.includes('workout')) {
          return 'Design my muscle-building routine';
        }
        if (suggestion.includes('protein') || suggestion.includes('nutrition')) {
      ...
      <div className={`max-w-[85%] ${message.type === 'user' ? 'order-2' : 'order-1'}`}>
        <div className={`flex items-start space-x-4 ${message.type === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
          ...
          <div className={`flex-1 ${message.type === 'user' ? 'text-right' : 'text-left'}`}>
              ...
              {message.type === 'ai' && !message.isError && (
                <div className="flex items-center mt-3 pt-3 border-t border-gray-100 text-xs text-gray-500">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="font-medium">Personalized for {userProfile?.name || 'you'}</span>
            ...
            {message.type === 'ai' && getPersonalizedSuggestions().length > 0 && (
